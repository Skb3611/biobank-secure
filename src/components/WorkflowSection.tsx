import { Play, Fingerprint, ShieldCheck, CreditCard, Cog, CheckCircle2, ArrowRight } from "lucide-react";
...
const WorkflowSection = () => {
  return (
    <section id="workflow" className="py-24 relative bg-gradient-to-b from-secondary/20 to-background">
      <div className="container px-4 md:px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            A streamlined process from fingerprint scan to successful transaction
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 md:gap-2">
            {steps.map((step, index) => (
              <div key={step.label} className="flex flex-col items-center relative group">
                <div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gradient-to-br ${step.color} p-0.5 group-hover:scale-110 transition-transform duration-300`}>
                  <div className="w-full h-full rounded-2xl bg-background flex items-center justify-center">
                    <step.icon className="w-7 h-7 md:w-8 md:h-8 text-primary" />
                  </div>
                </div>
                <span className="mt-3 font-display text-sm font-medium">{step.label}</span>
